...
use crate::controller::bucket::io::web_file::{
...
pub trait BucketFileTrait{
    ...
    where
        Self: Sized;
    // Maybe later.. fn from(details:VirtualFileDetails) -> Self where Self: Sized;
    //fn from(filename: &str, mime: &Mime) -> Result<Self, Self::Error> where Self: Sized;
